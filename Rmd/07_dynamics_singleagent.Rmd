...
- The **discount factor** $\beta$ together with the static **reward functions** $\lbrace (u_t(s_t,a_t) \rbrace$ determine the **objective function**
  $$
...
The objective is to pick the decision rule (or **policy function**) $P = \boldsymbol a^* = \lbrace a^*_1, ..., a^*_t \rbrace$ that solves
$$
\max_{\boldsymbol a} \ \mathbb E_{\boldsymbol s'} \Bigg[ \sum_{t=0}^{T} \beta^{t} u_{t}\left(s_t, a_{t}\right) \Bigg]
$$
...
- We expect static demand would be much more elastic
...
- Weather affects static utitilies but not transition probabilities
  ...
  - Itegration means: *"compute expected utility of Harold Zurcher before he opens the window"*
- Month of the year affects transition probabilities but not utilities
  - Buses are used more in the winter
  - Integration means: compute average (over months) transition probabilities 

> **Note 1**: you can always get the non-expected value function if you know the probability of raining or the transition probabilities by month



### Policy Function

Along with this value function is a corresponding **policy (or choice) function** mapping the state $s_t$ into HZ's optimal replacement choice $a_t$
$$
...
2. Randomness in the state
   ...
   - He always makes the best decision but based on some unobservable
...
- The $\epsilon_{it}$ are components of utility of alternative $a$ that are observed by HZ but not by us as econometrician.
...
- Now the **state** is $(s_t, \epsilon_t)$
  ...
  - State space is not  $k^3 = (k \text{ points})^{3 \text{ variables} \times 1 \text{ period}}$ 
...
1. **Curse of dimensionality in the state space**: ($s_t, \epsilon_{0t}, \epsilon_{1t}$)
   ...
   - Now, for the same grid size, there are $k^3$ : $k$ each for $s$, $\epsilon_0$, $\epsilon_1$
...
- **Lesson**: any state variable that does not affect continuation values (the future) does not have to be in the "actual" state spa
...
- **What it buys**
  - Closed for solution for $\mathbb E_{\epsilon_{t+1}}$
...
3. Init a choice-specific value function $\bar V^{(0)}_a (s_{t+1} ; \theta)$
   1. Apply the Bellman operator to compute $\bar V^{(1)}_a (s_{t+1} ; \theta)$
...
$$
\begin{aligned}
\mathbb E \Big[\epsilon_{1 t} \Big| a_{t}=1, \cdot\Big] &= \mathbb E \Big[\epsilon_{ t} | \bar{V_1}\left(s_t ; \theta\right)+\epsilon_{1 t}>\bar{V}_{0}\left(s_t ; \theta\right)+\epsilon_{0 t}\Big] \newline
...
-   **Assumption**: you have enough data
    ...
    -   Use frequencies in observed states to extrapolate frequencies in observed states
...
>  Why **pseudo-likelihood**? We have inputed something that is not a primitive but a consistent estitimate of an equilibrium object, $\hat P$
...
- The unobserved heterogeneity generates **persistent over choices**
...
- But hard to get given $\tau$ is not observed!
